...
//      DBCursor dbCursor=collection.find();
    ...
    public static void main(String[] args) {
        DB db = getDB("test");
        DBCollection sensorCollection = db.getCollection("sensorCollection");
        BasicDBObject query =new BasicDBObject();
        query.put("sensorNum", "301");
        List<DBObject> l = sensorCollection.find(query).toArray();

        for (DBObject o : l) {
            System.out.println(o.toMap().get("dataType"));
        }
